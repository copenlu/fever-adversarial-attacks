...
from builders.data_loader import sort_key, BucketBatchSampler, FeverDataset, collate_fever
from builders.model_builder import NLILSTM, NLICNN, EarlyStopping
...
if __name__ == "__main__":
    ...
    parser.add_argument("--embedding_dim", help="Dimension of embeddings", choices=[50, 100, 200, 300], default=100, type=int)
    ...
    parser.add_argument("--hidden_sizes", help="Number of units in the hidden layer", default=[100, 50], type=int, nargs='+')
    ...
    if args.mode == 'test':
        ...
        test_dl = BucketBatchSampler(batch_size=args.batch_size, sort_key=sort_key, dataset=test, collate_fn=collate_fn)
        ...
        train_dl = BucketBatchSampler(batch_size=args.batch_size, sort_key=sort_key, dataset=train, collate_fn=collate_fn)
